...
気泡中の酸素は気境膜ー気液界面ー液境膜を通過して液体に溶解する．境膜とは気液界面近傍にある境界層と考えればよく，気体側では圧力境界，液体側では濃度境界として表現される．酸素の移動はそれぞれの場所での移動容量係数を考えるが，一般的には酸素のような難溶解性気体の場合，液境膜部における移動容量係数が支配的であるため，液側物質移動容量係数$k_{\mathrm{L}}a$を総括物質移動容量係数${K_{\mathrm{L}}a}$として扱う．ここで$K_{\mathrm{L}}$は液境膜酸素移動係数[m/s]であり，$a$は気液接触面積[m-1]である．均一流において，気泡系$d$が既知であれば

$$
a = \frac{6 \varepsilon_{\mathrm{G}}}{d}
$${#eq:gas_surface_area}
である．$\varepsilon_{\mathrm{G}}$はガスホールドアップ（気泡塔の母液体積分率）である．すなわち，ボイド率（気泡塔の気相体積分率）を$\alpha$とすれば$\varepsilon_{\mathrm{G}} = 1 - \alpha$である．今回のような，均一流れかつ循環液流がない場合，ガスホールドアップは気泡のスリップ速度$u_{\mathrm{s}}$とガス空塔速度$u_{\mathrm{G}}$から，
...
である．ここで$C_{\mathrm{L}}$は液相の溶存酸素濃度，$C_{\mathrm{L}}^{*}$は液相の飽和溶存酸素濃度である．$C_{\mathrm{L}}^{*}$は空気供給装置の設置水深$h$と大気圧下での飽和溶存酸素濃度$C_{\mathrm{SA}}$から以下のように求められる．
...
さて，$K_{\mathrm{L}}a$は空塔速度や気泡径などの運転条件により影響を受け，一般的には実験的に求められるようである（2000年代以降，徐々に数値計算的なアプローチから求めることも行われているようであるが，まだ未調査である）．すなわち，[@eq:dC_dt]を積分することで
